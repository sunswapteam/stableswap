...
contract StableSwap3Pool is ReentrancyGuard{
    ...
    uint256[N_COINS] PRECISION_MUL = [1, 1, 1000000000000];
    ...
    uint256 constant RATES_1 = 1000000000000000000;
    uint256 constant RATES_2 = 1000000000000000000000000000000;
    ...
    function _xp() internal view returns (uint256[N_COINS] memory){
        uint256[N_COINS] memory result = [RATES_0, RATES_1, RATES_2];
        for (uint i = 0; i < N_COINS; i++) {
    ...
    function get_dy(uint128 i, uint128 j,uint256 dx) external view returns(uint256){
        uint256[N_COINS] memory rates = [RATES_0, RATES_1, RATES_2];
        uint256[N_COINS] memory xp = _xp();
        uint256 x = xp[i].add(dx.mul(rates[i]).div(PRECISION));
        uint256 dy = (xp[j].sub(get_y(i, j, x, xp)).sub(1)).mul(PRECISION).div(rates[j]);
        return dy.sub(fee.mul(dy).div(FEE_DENOMINATOR));
    }
