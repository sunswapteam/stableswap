...
import {SafeMath} from "../3pool/utils/SafeMath.sol";
import {ITRC20} from "../3pool/utils/ITRC20.sol";
import {TransferHelper} from "../3pool/utils/TransferHelper.sol";
import {ReentrancyGuard} from "../3pool/utils/ReentrancyGuard.sol";
...
contract StableSwap2Pool is ReentrancyGuard{
    ...
    uint256[N_COINS] ZEROS = [ZEROS256, ZEROS256];
    //
    ...
    //uint256[N_COINS] RATES = [1000000000000000000, 1000000000000000000, 1000000000000000000000000000000];
    uint256 constant RATES_0 = 1000000000000000000;
    uint256 constant RATES_1 = 1000000000000000000000000000000;

    uint128 constant FEE_INDEX = 1;

    uint256 constant MAX_ADMIN_FEE = 10 * 10 ** 9;
    uint256 constant MAX_FEE = 5 * 10 ** 9;
    uint256 constant MAX_A = 10 ** 6;
    uint256 constant MAX_A_CHANGE = 100;

    uint256 constant ADMIN_ACTIONS_DELAY = 3 * 86400;
    uint256 constant MIN_RAMP_TIME = 10 * 60;
    ...
    SSPLiquidityToken public token;
    ...
    constructor(
        ...
        token = SSPLiquidityToken(_pool_token);
    ...
    function _xp() internal view returns (uint256[N_COINS] memory){
        uint256[N_COINS] memory result = [RATES_0, RATES_1];
        for (uint i = 0; i < N_COINS; i++) {
    ...
    function _xp_mem(uint256[N_COINS] memory _balances) internal pure returns (uint256[N_COINS] memory){
        uint256[N_COINS] memory result = [RATES_0, RATES_1];
        for (uint i = 0; i < N_COINS; i++) {
    ...
    function get_D(uint[N_COINS] memory xp,uint amp) internal pure returns (uint256) {
        require(N_COINS == xp.length, "N_COINS not eq xp.length");
        uint256 S = 0;
        ...
        uint Dprev = 0;
        uint D = S;
        uint Ann = amp * N_COINS;
        for (uint i = 0; i < 255; ++i) {
            uint D_P = D;
            for (uint k = 0; k < N_COINS; ++k) {
                D_P = D_P.mul(D).div(xp[k] * N_COINS);
            }
            Dprev = D;
            D = (Ann * S).add(D_P.mul(N_COINS)).mul(D).div((Ann - 1).mul(D).add((N_COINS + 1).mul(D_P)));
            if( D > Dprev){
                if (D - Dprev <= 1) {
                    break;
                }
    ...
    function get_dy(uint128 i, uint128 j,uint256 dx) external view returns(uint256){
        uint256[N_COINS] memory rates = [RATES_0, RATES_1];
        uint256[N_COINS] memory xp = _xp();
        uint256 x = xp[i].add(dx.mul(rates[i]).div(PRECISION));
        uint256 dy = xp[j].sub(get_y(i, j, x, xp)).sub(1);
        return (dy.sub(fee.mul(dy).div(FEE_DENOMINATOR))).mul(PRECISION).div(rates[j]);
    }
    ...
    function remove_liquidity_imbalance(uint256[N_COINS] calldata amounts, uint256 max_burn_amount) external nonReentrant{
        ...
        uint256 amp = _A();
        uint256[N_COINS] memory old_balances = balances;
        uint256[N_COINS] memory new_balances = _cp_array(balances);
        uint256 D0 = get_D_mem(old_balances, amp);
    ...
    function get_y_D(uint256 A_, uint128 i, uint256[N_COINS] memory xp, uint256 D) internal pure returns(uint256){
      /**
    ...
    function _calc_withdraw_one_coin(uint256 _token_amount, uint128 i) internal view  returns(uint256,uint256){
        ...
        dy = (dy.sub(1)).div(PRECISION_MUL[i]);

        return (dy, dy_0.sub(dy));
    }

    function _cp_array(uint256[N_COINS] memory _balances) internal pure returns (uint256[N_COINS] memory){
        uint256[N_COINS] memory result;
        for (uint i = 0; i < N_COINS; i++) {
            result[i] = _balances[i];
        }
        return result;
    }
    ...
    function calc_withdraw_one_coin_with_fee(uint256 _token_amount, uint128 i) external view returns (uint256, uint256) {
